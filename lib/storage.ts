...
export function getProfile(): UserProfile | null {
  ...
  try { const raw = localStorage.getItem(PROFILE_KEY); return raw ? (JSON.parse(raw) as UserProfile) : null; } catch { return null; }
}
...
export function getAllLogs(): Record<string, DayLog> {
  if (!isBrowser) return {};
  try { const raw = localStorage.getItem(LOGS_KEY); return raw ? (JSON.parse(raw) as Record<string, DayLog>) : {}; } catch { return {}; }
}
...
export function getLast7Days(): DayLog[] {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    result.push(logs[ds] ?? { date: ds, meals: [], workout: null });
  }
  return result;
}
