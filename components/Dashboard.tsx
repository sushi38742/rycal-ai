...
function CalorieHero({ remaining, total }: { remaining: number; total: number }) {
  ...
  const isNear = remaining >= 0 && remaining < total * 0.1;
  const color = isOver ? 'text-rose-400' : isNear ? 'text-amber-400' : 'text-white';
  return (
    <div className="flex flex-col items-center py-10">
      <div className={'text-[88px] leading-none font-black tabular-nums tracking-tighter transition-colors duration-300 ' + color}
        style={{ textShadow: isOver ? '0 0 60px rgba(244,63,94,0.3)' : '0 0 80px rgba(249,115,22,0.15)' }}>
        {Math.abs(remaining)}
      </div>
      <p className="text-gray-500 text-sm font-medium mt-2 tracking-wide">{isOver ? 'calories over' : 'calories remaining'}</p>
...
function CameraButton({ onClick }: { onClick: () => void }) {
  return (
    <div className="flex flex-col items-center py-8">
      <button type="button" onClick={onClick} className="relative group focus:outline-none" aria-label="Log a meal with photo">
    ...
    <div className="min-h-screen bg-black text-white">
      <header className="flex items-center justify-between px-6 pt-12 pb-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="button" onClick={() => { if (confirm('Reset all data and redo setup?')) { localStorage.clear(); window.location.href = '/setup'; } }}
            className="p-1.5 rounded-xl text-gray-700 hover:text-gray-400 transition-colors" aria-label="Settings">
            <Settings className="w-4 h-4" />
          </button>
        </div>
      </header>
      <CalorieHero remaining={remaining.calories} total={profile.dailyCalories} />
      <div className="mb-6"><MacroBars profile={profile} consumed={consumed} /></div>
      <CameraButton onClick={() => setShowLogger(true)} />
      <div className="mb-8"><WorkoutTracker date={today} dayLog={dayLog} onUpdate={refresh} /></div>
      <div className="mb-10">
        <div className="flex items-center justify-between px-6 mb-4">
          <h2 className="text-xs font-bold text-gray-500 uppercase tracking-widest">Today's Meals</h2>
          {dayLog.meals.length > 0 && <span className="text-xs text-gray-600">{dayLog.meals.length} logged</span>}
        </div>
        <MealLogList meals={dayLog.meals} onDelete={(id) => { removeMeal(id, today); refresh(); }} />
      </div>
      {showLogger && <PhotoLogger onClose={() => setShowLogger(false)} onMealLogged={() => { refresh(); setShowLogger(false); }} />}
    </div>
  );
}
